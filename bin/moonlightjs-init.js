...
program
  ...
  .option('--offline [value]', 'use cached template or specific a local path to moonlightjs-template')
  ...
  .on('--help', () => {
    ...
    console.log('    $ moonlightjs init awesome-project')
    console.log()
    console.log(chalk.gray('    # create a new project in current directory'))
    console.log('    $ moonlightjs init')
    console.log()
  })
  .parse(process.argv)
...
let template = 'moonlightjs-template'
...
const tmp = path.join(home, '.moonlightjs-templates', template)
if (program.offline) {
...
function run () {
  ...
  if (isLocalPath(template)) {
    ...
    checkVersion(() => {
      ...
      const officialTemplate = 'wrfighting/' + template
